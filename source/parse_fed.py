...
OUT_NM = './federal_data_.c'
APPOINTERS_CSV = '../data/appointers.csv'
DECISIONS_CSV = '../data/decisions.fed-circuit.csv'
...
ROW_APPT = '^'+','.join((     ','.join(STR for _ in range(2)),
                         GRP, GRP, GRP, GRP, ','.join(STR for _ in range(13)),
...
"Last Name","First Name","Middle Name","Suffix",
    "Birth Month","Birth Day","Birth Year","Birth City","Birth State",
    "Death Month","Death Day","Death Year","Death City","Death State","Gender",
    "Race or Ethnicity","Court Type (1)",
"Court Name (1)",
...
"Party of Appointing President (1)",
    ...
    "Senate Vote Type (1)","Ayes/Nays (1)","Confirmation Date (1)",
"Commission Date (1)",
    "Service as Chief Judge, Begin (1)","Service as Chief Judge, End (1)",
...
"Party of Appointing President (2)",
    ...
    "Senate Vote Type (2)","Ayes/Nays (2)","Confirmation Date (2)",
"Commission Date (2)",
    "Service as Chief Judge, Begin (2)","Service as Chief Judge, End (2)",
    "2nd Service as Chief Judge, Begin (2)",
...
def find_judges_of_court(court_nm):
    parties = {}
    tbegs   = {}
    tends   = {}

    with open(APPOINTERS_CSV) as f:
        lns = f.read().split('\n')[1:]
    for ln in lns:
        # NOTE: the court of interest may be mentioned in irrelevant rows, e.g.
        # if it appears under clerkships, so we get a superset
        if court_nm not in ln: continue

        (last_name, first_name, middle, suffix,
             court0, pres0, dbeg0, dend0,
             court1, pres1, dbeg1, dend1,) = re.match(ROW_APPT, ln).groups()

        # NOTE: we DON'T do merely `if court0 != court_nm: continue` since
        # judges can be reassigned from a different court to the court of
        # interest
        ...
        # NOTE: judges may share surnames (!two `John Marshall Harlan`s have
        # served on SCOTUS!)... so we also use suffixes etc
        # TODO
        #nm = first_name.strip()[:1]+middle.strip()[:1]+'.'+last_name+suffix
        nm = last_name

        dbeg = term_from_date(dbeg)
        dend = term_from_date(dend)
        print('{:16s}{:10s} --- {:4d}-{:4d} --- {}'.format(
              nm,pres,dbeg,dend,court))
        parties[nm] = pres
        tbegs  [nm] = dbeg
        tends  [nm] = dend

    return parties,tbegs,tends

if __name__=='__main__':
    parties,tbegs,tends = find_judges_of_court(COURT_NM)
